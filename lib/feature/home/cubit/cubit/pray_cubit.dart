...
class PrayCubit extends Cubit<PrayState> {
  ...
  //   var result =
  //       await prayTimeRepo.fetchPrayTime(date: getDate(0), country: "egypt");
  //   try {
  //     result.fold(
  //       (l) {
  //         emit(PrayError(l.errorMassage));
  ...
  calculateNextPray(PrayModel r) {
    ...
    if (nextPrayer == null) {
      ...
      nextPrayer = prayerDateTimes.first.add(Duration(days: 1));
  ...
  @override
  void onChange(Change<PrayState> change) {
    log(change.toString());
